...
|---|---|---|
| **ActivityEvent** | `event_type`, `message`, `agent_id`, `task_id`, `board_id`, `created_at` — activity coverage across agent lifecycle, task status, sprints, planning artefacts, approvals, file writes/deletes, and board lead notifications | Postgres `activity_events` |
...
| **Channels & Threads** | Board-scoped messaging channels (`alert`/`discussion`, plus direct-channel routing support), threads linked to tasks, thread messages, agent subscriptions (`all`/`mentions`/`none`), per-user read state, and `owner_board_id` support for cross-board support-thread isolation | Postgres `channels`, `threads`, `thread_messages`, `channel_subscriptions`, `user_channel_states` |
...
| **Gateway RPC** | Protocol v3 with 90 known base methods across usage, sessions, config, exec/approvals, agents, files, skills, MCP apps, cron, node/device pairing, chat, TTS; 18 known event types | Transient — fetched live per request, never persisted |
...
| **Heartbeat** | Agent check-in updates `last_seen_at` and emits `agent.heartbeat` activity events | Postgres `agents` + `activity_events` |
| **Queue** | Redis-backed list queue with delayed scheduling, scheduled-task drain, and exponential backoff retry with jitter. Active worker dispatch covers `agent_lifecycle_reconcile` and `webhook_delivery`; an `agent_webhook_delivery` payload/processor also exists but is not currently registered in the generic worker dispatch map | Redis + Postgres |
...
2. **No per-agent cost attribution** — the gateway reports aggregate usage, not clearly persisted per-session or per-agent.
3. **No structured audit trail** — `activity_events` captures high-level event types but not the granular causal chain (prompt → tool call → approval → execution → result) and carries no token/cost enrichment.
...
5. **Governance primitives partially addressed** — board-level workflow controls (approval gates, review requirements, lead-only status changes), per-agent skill allow-lists, and support-thread ownership isolation already exist, but there are no budget limits, model allow-lists, session TTL enforcement, or compliance export capabilities.
6. **Channel/thread/message activity not audited** — no `record_activity` calls for channel creation, thread creation, message posting, read-state changes, mute changes, or subscription changes.
7. **Skill install/uninstall events not recorded** — skill lifecycle changes produce no activity events.
8. **Board governance setting changes not recorded** — board update code computes `changed_fields`, but governance-flag changes are not explicitly emitted to the activity stream.
9. **MCP app invocations opaque** — `mcp.tools.call` passes through the gateway with no Product Foundry visibility or logging.
10. **Queue dispatch is slightly inconsistent** — the queue layer already supports additional payload types, but not every defined queue task is wired into the generic worker loop yet.
...
Protocol v3 currently exposes 90 known base methods in this codebase. Key categories for telemetry:
...
| **Exec / Approvals** | `exec.approvals.get`, `exec.approvals.set`, `exec.approvals.node.get`, `exec.approvals.node.set`, `exec.approval.request`, `exec.approval.resolve` | Approval rules and resolution | Governance evidence |
...
| **MCP Apps** | `mcp.tools.list`, `mcp.tools.call`, `mcp.resources.list`, `mcp.resources.read` | MCP tool invocations and resource access | New audit surface |
...
Gateway events (18 known types): `connect.challenge`, `agent`, `chat`, `presence`, `tick`, `talk.mode`, `shutdown`, `health`, `heartbeat`, `cron`, `node.pair.requested`, `node.pair.resolved`, `node.invoke.request`, `device.pair.requested`, `device.pair.resolved`, `voicewake.changed`, `exec.approval.requested`, `exec.approval.resolved`.
...
|---|---|---|
| **Channels** | Channel creation, thread creation, message posting, subscription changes, mute/read-state changes | Not logged via `record_activity` |
...
| **Board governance** | Approval gate changes, review requirement changes, lead assignment, related board settings | Only partial logging today; setting changes themselves are not explicitly recorded |
| **MCP app results** | Chart renders, app metadata writes to board memory and thread messages | Not logged as audit events; stored as content records only |
...
├── gateway_id       UUID FK → gateways (nullable)
...
Design note: audit rows should be resilient to deletion of mutable operational records. In practice this likely means nullable FKs plus denormalized identifiers/labels, or `ON DELETE SET NULL`, rather than strict append-only rows tightly coupled to `agents`, `boards`, or `tasks` lifecycle.
...
1. Add scheduled background work (using the existing queue/scheduler infrastructure) that calls `usage.status` + `usage.cost` per gateway.
2. Store snapshots in a new `usage_snapshots` table keyed effectively by `(gateway_id, agent_id, session_key, model_id, captured_at)` so concurrent per-model rows at the same timestamp remain lossless.
...
2. Extend `record_activity()` to dual-write into `agent_audit_log` with `source='product_foundry'`.
3. Add `record_activity` calls to currently unlogged events: channel/thread creation, messages, mute/read-state transitions, skill installs/uninstalls, agent skill allow-list updates, MCP app calls, board governance setting changes.
4. Extend heartbeat payloads so agents can optionally self-report token/cost/session telemetry.
...
2. Alternatively, add a new OpenClaw hook that POSTs directly to Product Foundry on each command execution.
...
1. Subscribe to gateway events (`system-event`, `tick`, `heartbeat`, `exec.approval.requested`, `exec.approval.resolved`, etc.) via a persistent WebSocket listener.
...
Implementation note: treat these rows as cumulative snapshots unless the gateway exposes interval deltas explicitly. Aggregations will likely need snapshot differencing rather than raw summation.
...
| **Per task** | Prefer direct `task_id` linkage in audit rows; use `correlation_id` only as a grouping aid |
...
| **Budget alerts** | Webhook/email/channel notification at 50%, 80%, 100% | Background job checks `usage_snapshots` vs caps; can post to alert channels |
...
**Proposed extension** — add two or more new roles, but implement authorization as a capability system rather than pure rank-based gating:

| Role | Baseline intent |
|---|---|
| **owner** | Full control, billing, member management |
| **admin** | Gateway management, agent lifecycle, project CRUD |
| **operator** | Agent management within assigned projects, approval resolution |
| **viewer** | Read-only dashboard, activity feed, project views |
| **auditor** | Read-only access to audit logs, usage data, compliance exports — no operational actions |

Implementation:
1. Extend `ROLE_RANK` only for coarse ordering if helpful, but do not rely on rank alone.
2. Introduce explicit capability checks (for example: `manage_org`, `manage_gateways`, `resolve_approvals`, `view_audit`, `export_compliance`) because `viewer` and `auditor` have overlapping rank but different permissions.
3. Replace `require_org_admin` incrementally with permission-aware dependencies, using rank only as one input to policy evaluation.
4. Frontend: role-aware navigation and page-level capability gating.
...
| **Retention policy** | Configurable per-org retention period; background job prunes older rows |
| **Config snapshots** | Periodic `config.get` snapshots stored for change tracking |
| **Approval evidence chain** | Link `approval.id` → `agent_audit_log.correlation_id` and/or direct audit identifiers so every approval can be traced to the triggering command and resulting execution |
...
| **Thread privacy** | Foundation already exists via `threads.owner_board_id` for cross-board support-thread isolation; extend this into broader channel/thread ACLs if needed |

### 6.5 Agent Governance Policies

| Policy | Description | Enforcement |
|---|---|---|
| **Max agents per project** | Prevent runaway agent creation | Already exists: `Board.max_agents` column and `AgentCreateLimitService`. Extend to configurable per-org override |
| **Mandatory approval for high-risk actions** | Require human sign-off for destructive commands | Extend `exec.approvals` integration; Product Foundry stores policy, gateway enforces. Board-level approval gates already exist |
...
| **Session TTL / compaction policy** | Enforce max session age before forced compaction/reset | Org-level setting; scheduled job calls `sessions.compact` for stale sessions |
| **Skill allow-list / deny-list** | Control which skills can be installed | Per-agent: `Agent.installed_skills` already exists as a JSON allow-list. Per-org: add `skill_deny_list` to org settings JSONB. `MarketplaceSkill.risk` supports risk-based gating |
...
|---|---|
| **M1** | `CREATE TABLE agent_audit_log (...)` — include `gateway_id`, `task_id`, `thread_id`, `sprint_id`, expanded `event_category` values, and deletion-safe FK semantics for append-only retention |
| **M2** | `CREATE TABLE usage_snapshots (...)` — support per-model snapshot rows at a shared capture timestamp |
...
| **M6** | Keep `organization_members.role`; extend role/capability logic in service code and schemas to add `operator`, `viewer`, `auditor` behavior |
| **M7** | Extend `organizations.settings` schema to include `skill_deny_list`, model policy settings, retention settings, and governance config |
...
|---|---|---|
| **Phase 1** | Usage snapshots table + scheduled usage poll job + dashboard cost widget | 1 sprint |
| **Phase 2** | `agent_audit_log` table + dual-write from `record_activity` + extend `record_activity` to channels/skills/MCP/board-governance + audit list API + frontend audit trail page | 1–2 sprints |
| **Phase 3** | Command-logger ingestion (sidecar or hook) + real-time event subscription | 1–2 sprints |
| **Phase 4** | RBAC role/capability extension (`operator`/`viewer`/`auditor`) + governance policy CRUD + budget controls + auto-pause | 2 sprints |
...
1. **Gateway per-session cost** — Does the OpenClaw gateway expose per-session token counts, or only aggregate? If aggregate-only, we need agent self-reporting or snapshot differencing.
2. **Command-logger format** — What is the exact JSON schema of `command-logger` output? Need a sample to build the parser.
3. **Event push vs poll** — Can we subscribe to a gateway event stream for `usage` changes, or must we poll?
...
6. **Upstream hook API** — Would OpenClaw accept a new webhook-push hook type that POSTs command logs directly to a configurable URL?
7. **Channel message volume** — At scale, should thread messages be fully logged, sampled, or reduced to metadata only?
8. **MCP app audit granularity** — Should every `mcp.tools.call` be individually logged, or only failures/high-cost calls?
9. **Sprint velocity ↔ cost correlation** — Should velocity snapshots feed cost-per-story-point or cost-per-sprint metrics?
10. **Queue unification** — Should the existing `agent_webhook_delivery` path be wired into the generic worker before adding more telemetry/background task types?
11. **Deletion semantics** — Should audit/usage rows retain soft references only, or preserve hard FKs with `SET NULL` semantics for deleted agents/boards/tasks?
...
| Gateway event subscription (persistent WS) | Product Foundry backend | New background worker/listener pattern |
| Queue worker alignment | Product Foundry backend | Existing queue supports delayed scheduling/retries; task registration should be unified before adding more background telemetry types |
