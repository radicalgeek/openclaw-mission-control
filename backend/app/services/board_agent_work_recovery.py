...
def _lead_wake_message(
    *,
    ...
    return (
        ...
        "proceed; make assignment decisions now when inbox work is ready. Assign developers "
        f"with PATCH `/api/v1/agent/boards/{board.id}/tasks/{{task_id}}` and JSON "
        '{"assigned_agent_id":"<developer_agent_id>"}. Do not use OpenClaw direct-message '
        "tools, session labels, `sessions.resolve`, `dev-1`, `dev-2`, `dev-agent-1`, or "
        "`dev-agent-2` for developer assignment; those labels may not exist. Assignment via "
        "AxiaCraft wakes the worker automatically. Make the board-lead decision using "
