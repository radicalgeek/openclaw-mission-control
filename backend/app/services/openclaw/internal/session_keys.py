...
Session keys are part of Product Foundry's contract with the OpenClaw gateway.
...
def standalone_agent_session_key(agent_id: UUID) -> str:
    """Return the deterministic session key for a standalone agent."""
    return f"{AGENT_SESSION_PREFIX}:standalone-{agent_id}:main"
