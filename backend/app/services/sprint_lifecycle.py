...
def _build_sprint_started_lead_message(
    *,
    ...
    return (
        ...
        "this turn to complete this mandatory assignment loop:\n"
        ...
        "4. Choose a balanced distribution across available non-lead developer agents.\n"
        "5. Assign every unassigned sprint inbox ticket with PATCH "
        f"{task_update_path} using "
        '{"assigned_agent_id":"<developer_agent_id>"}.\n'
        f"6. Verify assignment by re-reading GET {task_list_path}; finish only when "
        "there are zero unassigned sprint inbox tickets or you have posted a concrete "
        "blocker to board chat.\n\n"
        "Assignment is an AxiaCraft API write, not a chat/message operation. Do not "
        "use OpenClaw direct-message tools, channel/message tools, session labels, or "
        "`sessions.resolve` to reach developers for this assignment cycle. If any "
        "message/channel tool returns `Action send requires a target`, `Channel is "
        "required`, or any similar routing error, abandon that path immediately and "
        f"use PATCH {task_update_path} instead. "
