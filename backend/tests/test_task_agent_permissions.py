...
@pytest.mark.asyncio
async def test_standalone_estimator_can_set_backlog_estimate() -> None:
    engine = await _make_engine()
    try:
        async with await _make_session(engine) as session:
            org_id = uuid4()
            ...
            updated = await tasks_api.update_task(
                payload=TaskUpdate(
                    ...
                    comment="Estimate: 120 minutes. Comparable moderate backend change.",
                ),
                task=task,
                session=session,
                actor=ActorContext(actor_type="agent", agent=actor),
            ...
            assert [c.message for c in comments] == [
                "Estimate: 120 minutes. Comparable moderate backend change."
            ]
    finally:
        await engine.dispose()
...
@pytest.mark.asyncio
async def test_estimator_cannot_set_estimate_on_kanban_task() -> None:
    engine = await _make_engine()
    try:
        async with await _make_session(engine) as session:
            org_id = uuid4()
            ...
            with pytest.raises(HTTPException) as exc:
                await tasks_api.update_task(
                    payload=TaskUpdate(estimate_minutes=120),
                    task=task,
                    session=session,
                    actor=ActorContext(actor_type="agent", agent=actor),
                )
...
async def test_lead_done_transition_records_actual_minutes_from_in_progress_start() -> (
    ...
    try:
        async with await _make_session(engine) as session:
            ...
            lead_id = uuid4()
            task_id = uuid4()
            started_at = utcnow() - timedelta(minutes=89, seconds=10)
