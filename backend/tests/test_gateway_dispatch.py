...
@pytest.mark.asyncio
async def test_wake_agent_session_retries_session_patch_after_reset_transport_error(
    monkeypatch,
) -> None:
    calls: list[tuple[str, dict[str, object]]] = []
    patch_attempts = 0
    ...
    async def _ensure_session(
        ...
        *,
        ...
        if patch_attempts == 1:
            raise gateway_dispatch.OpenClawGatewayError("received 1000 (OK); then sent 1000 (OK)")
        return {}
    ...
    async def _send_session_message_nonblocking(
        ...
        *,
        ...
        calls.append(("sessions.send", {"key": session_key, "message": message}))
        return {}
    ...
    assert calls == [
        ...
        (
            "sessions.patch",
            {"key": "agent:lead:main", "label": "Lead Agent", "attempt": 1},
        ),
        (
            "sessions.patch",
            {"key": "agent:lead:main", "label": "Lead Agent", "attempt": 2},
        ),
        ("sessions.send", {"key": "agent:lead:main", "message": "wake up"}),
    ]
